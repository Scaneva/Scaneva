#region Copyright (C)
// ---------------------------------------------------------------------------------------------------------------
//  <copyright file="PC_PI_E_727_3x.cs" company="Scaneva">
// 
...
{
    [DisplayName("PI E-727 piezo controller")]
    [Category("Positioner")]
    class PC_PI_E_727_3x : ParametrizableObject, IHWManager, IPositioner
    {
		...
		*/
        ...
        [DllImport("PI_GCS2_DLL.dll", EntryPoint = "PI_qVLS")] public static extern int VLS(int ID, double dSystemVelocity);
        ...
        [DllImport("PI_GCS2_DLL.dll", EntryPoint = "PI_qNAV")] public static extern int qNAV(int ID, int[] iAnalogChannelIds, int[] iNrReadingsValues, int iArraySize);
        ...
        int[] bIsMoving = new int[4];
        int[] bFlags = new int[4];
        int[] iVal = new int[4];
        int[] iChnl = new int[4];
        double[] dPos = new double[4];


        public PC_PI_E_727_3x(LogHelper log) : base(log)
        {
            settings = new PC_PI_E_727_3x_Settings();
        }
       ...
       // public string Name => m_strName;
        ...
        public enuHWStatus Initialize()
        {
            ...
            EnumerateUSB(sUsbController, 1024, "PI E-727");
            m_iControllerId = ConnectUSB(sUsbController.ToString());

            if (m_iControllerId < 0)
            {
                EvaluateError();
                return enuHWStatus.Error; //todo: translate positioner status to HW status
            }

            //get device identification
            if (qIDN(m_iControllerId, sIdn, sIdn.Capacity) == 0)
            {
                EvaluateError();
                return enuHWStatus.Error; //todo: translate positioner status to HW status
            }

            log.Add("> IDN?:" + sIdn.ToString());

            // Get the name of the connected axis.
            if (qSAI(m_iControllerId, sAxes, sAxes.Capacity) == 0)
            {
                EvaluateError();
            ...
            /*
            if (qATZ(m_iControllerId, axes, ) == 0)
            {
        ...
        public void Release()
        {
            if (HWStatus.HasFlag(enuHWStatus.Ready) && !HWStatus.HasFlag(enuHWStatus.NotInitialized) && !HWStatus.HasFlag(enuHWStatus.Error))
            {
                if (m_iControllerId >= 0)
                    CloseConnection(m_iControllerId);
            }
        }

        public Position AbsolutePosition()
        {
            Position tpos = new Position(0,0,0);
            return tpos;
        }
        ...
        public enuPositionerStatus AbsolutePosition(Position _pos)
        {
                        ...
                        // Get the name of the connected axis.
            if (qSAI(m_iControllerId, sAxes, sAxes.Capacity) == 0) return EvaluateError(); //TODO: evaluate iError and return correct enuPositionerStatus
            ...
            if (MOV(m_iControllerId, axes, dPos) == 0)
            {
        ...
        private enuPositionerStatus EvaluateError()
        {
            m_Error = GetError(m_iControllerId);
            TranslateError(m_Error, sErrorMessage, sErrorMessage.Capacity);
           log.Add(this.ToString() + " ERROR " + m_Error.ToString() + ": " + sErrorMessage.ToString());
